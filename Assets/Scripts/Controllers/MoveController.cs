...
public class MoveController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            rigidbodies[i].linearVelocity = new Vector2(moveInput * persons[i].movementSpeed * persons[i].movementDirection, rigidbodies[i].linearVelocity.y);
        }
        ...
        if (Input.GetButtonDown("Jump"))
        {
            for (int i = 0; i < persons.Count; i++)
            {
                rigidbodies[i].linearVelocity = new Vector2(rigidbodies[i].linearVelocity.x, persons[i].jumpForce);
            }
        }
    }
